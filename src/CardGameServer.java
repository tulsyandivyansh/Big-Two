...
public class CardGameServer {
	...
	private final ObjectOutputStream[] clientOutputStreams;
	...
	private final boolean[] clientReadyStates;
	...
	public CardGameServer(String serverName, int maxNumOfPlayers) {
		...
		if (maxNumOfPlayers <= 0) {
			throw new IllegalArgumentException("Maximum player count must be positive");
		}
	...
	private synchronized void addPlayer(Socket clientSocket, String name) {
		if (name == null || name.trim().isEmpty()) {
			println("Rejected a player with an empty name.");
			return;
		}
		if (numOfPlayers > 0) {
			// locates the client socket in the array
	...
	private void closeQuietly(Socket socket) {
		try {
			...
			// The socket is already closed or unusable.
		}
	}
