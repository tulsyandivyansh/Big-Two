...
public class CardList implements Serializable {
	...
	/** Creates a shallow copy of another card list. Card values are immutable. */
	protected CardList(CardList source) {
		if (source == null) {
			throw new IllegalArgumentException("source must not be null");
		}
		cards.addAll(source.cards);
	}
	...
	public String toString() {
		...
		if (cards.size() > 0) {
			for (int i = 0; i < cards.size(); i++) {
				string.append('[').append(cards.get(i)).append(']');
				if (i != cards.size() - 1) {
					string.append(' ');
				}
			}
